...
int test = 0; // t and T
int textureIndex = 0;
int texOrRGBA = 0; // Controls using RGBA or texture
int perspective_correction = 0; // Controls if we're using perspective correction or not
int current[800][800];
float depth[800][800];
...
void colormap(IMAGE *image) {  // *image is where the IMAGE is saved
    image->width = 800;
    image->height = 800;
    for (int y = 0; y < image->height; y++) {
        for (int x = 0; x < image->width; x++) {
            float x_ratio = (float)x / (float)image->width;
            float y_ratio = (float)y / (float)image->height;
            UCset_vector(image->data[x][y], y_ratio * x_ratio * 255, (1 - x_ratio) * 255, y_ratio * 255, 255);
        }
    }
    // write_ppm(&Textures[0], "../Images/checkerboard", 0); // saves the IMAGE to a given file location with file name
}
...
void set_triangles(void) {
    ...
    for (int col = 0; col < MESH_WIDTH - 1; col++) { // sets triangles
        for (int row = 0; row < MESH_HEIGHT - 1; row++) {
            ...
            calc_normal(&triangle_list[num_triangles+1]); // sets the normal vector for the second triangle
            num_triangles += 2;
        }
    }
}
...
void draw_RGBApoint_struct(POINT p) {
    ...
    if (perspective_correction == 1) {
        ...
        scalar_multiply_vector(p.pos[Z], p.color, p.color);
        scalar_multiply_vector(p.pos[Z], p.tex_coords, p.tex_coords);
    } else {
    }

    if (p.pos[Z] > depth[x][y]) { // checks if the point is closer than the previous point
        return;
    }
    ...
    if (texOrRGBA == 0) {
        ...
        POINT copy;
        texture_sample(&Textures[textureIndex], p.tex_coords[V], p.tex_coords[U], copy.color); // samples the texture
        // texture_sample(&Textures[textureIndex], 0, 0, copy.color);
        set_vector(colorbuffer[x][y], p.color[R], p.color[G], p.color[B], p.color[A]); // sets the color buffer to the color of the point
        multiply_vector(colorbuffer[x][y], copy.color, colorbuffer[x][y]);
        // copy_vector(copy.color, colorbuffer[x][y]);
    }
}
...
int is_clockwise(POINT a, POINT b, POINT c)

{
       return (b.pos[X] - a.pos[X]) * (c.pos[Y] - a.pos[Y]) - (c.pos[X] - a.pos[X]) * (b.pos[Y] - a.pos[Y]) < 0;
}
...
void draw_barycentrictriangle( POINT p0, POINT p1, POINT p2 )
{
    ...
    for( int y = miny-1; y < maxy+2; y++ )
    {
        for( int x = minx-1; x < maxx+2; x++ )
        {
             ...
             */
            w[0] = edgeFunction(v2->pos, v1->pos, p.pos);
            w[1] = edgeFunction(v0->pos, v2->pos, p.pos);
            w[2] = edgeFunction(v1->pos, v0->pos, p.pos);
            w[3] = 0.0;
            ...
            if( w[0] >= 0 && w[1] >= 0 && w[2] >= 0 )
            {
                 ...
                 */
                float   inv_area = 1.0/(w[0] + w[1] + w[2]);
				scalar_multiply_vector( inv_area, w, w );
                // printf("w[0], w[1], w[2] = %f, %f, %f\n", w[0], w[1], w[2]);
...
/*
 * Calculates the Vector Angle between the normal vector and the light vector
 */
...
void project_model(void) {  // Flat 3D -> 2D projection of the 3D model
    for (int i = 0; i < num_verts; i++) {
...
void perspectiveproject_model(float  nearplane, float farplane) { // nearplane is the hypothetical screen
    for (int i = 0; i < num_verts; i++) {
        ...
        float z = vertex_list[i].world[Z] + BIAS;
        // printf("near = %f, far = %f, x = %f, y = %f, z = %f\n", nearplane, farplane, vertex_list[i].world[X], vertex_list[i].world[Y], vertex_list[i].world[Z]);

        vertex_list[i].pos[X] = (nearplane * x) / z;
        vertex_list[i].pos[Y] = (nearplane * y) / z;
        vertex_list[i].pos[Z] = z / (farplane - nearplane);
        // vertex_list[i].pos[Z] = (farplane - nearplane) / z;
        vertex_list[i].pos[W] = 1.0;
        // printf("X = %f, Y = %f, Z = %f\n", vertex_list[i].pos[X], vertex_list[i].pos[Y], vertex_list[i].pos[Z]);

        // Perspective transformation
        if (perspective_correction == 1) {
            vertex_list[i].pos[Z] = 1.0 / vertex_list[i].pos[Z];

            scalar_multiply_vector(vertex_list[i].pos[Z], vertex_list[i].color, vertex_list[i].color);
            scalar_multiply_vector(vertex_list[i].pos[Z], vertex_list[i].tex_coords, vertex_list[i].tex_coords);
            // printf("After X = %f, Y = %f, Z = %f\n\n", vertex_list[i].pos[X], vertex_list[i].pos[Y], vertex_list[i].pos[Z]);
        }
    }
}

void scale_viewport(int scale_x, int scale_y) {
    for (int i = 0; i < num_verts; i++) {
...
void draw_model(void) { // draws a filled model
    ...
    for (int t = 0; t < num_triangles; t++) {
        ...
        float brightness = dotProduct(triangle_list[t].normal_vector, light_vector);
        ...
        draw_barycentrictriangle(p0, p1, p2); // draw_barycentric is counter clockwise
    }
}
...
/*
 * init_sphere() sets all the vertices and triangles for drawing a sphere
 */
void init_sphere(float center_x, float center_y, float center_z) { // creates a sphere with radius 1
    num_verts = 0;
    for (int col = 0; col < MESH_HEIGHT; col++) { // sets points
        for (int row = 0; row < MESH_WIDTH; row++) {
            float u, v, x, y, z;
            u = (float)col / (MESH_HEIGHT - 1);
            v = (float)row / (MESH_WIDTH - 1);
            y = cos(u * PI) + center_x; // (0, 1) to (pi, -1) this is treated as the Z
            x = cos(v * 2 * PI) * sin(u * PI) + center_y; // this is treated as the x
            ...
            set_vector(vertex_list[num_verts].tex_coords, v, u, 0, 0);
            num_verts++;
        }
    }
...
void init_quad() {
    ...
    triangle_list[1].index_list[0] = 0;
...
void oneLine(void) {
    POINT p0, p1;
    set_vector(p0.pos, -200, -200, 0, 1);
    set_vector(p0.color, 1, 0, 0, 1);
    set_vector(p1.pos, 200, 200, 0, 1);
    set_vector(p1.color, 0, 0, 1, 1);
    draw_line_color(p0, p1);
}

void oneTriangle(void) {
    POINT p0, p1, p2;
    set_vector(p0.pos, -200, -200, 0, 1);
    set_vector(p0.color, 1, 0, 0, 1);
    set_vector(p1.pos, 200, -200, 0, 1);
    set_vector(p1.color, 0, 1, 0, 1);
    set_vector(p2.pos, 0, 200, 0, 1);
    set_vector(p2.color, 0, 0, 1, 1);
    draw_barycentrictriangle(p0, p1, p2);
}

void oneSquare_Text(void) {
    POINT p0, p1, p2, p3;
    set_vector(p0.pos, -200, -200, 0, 1);
    set_vector(p0.tex_coords, 0, 1, 0, 0);
    set_vector(p0.color, 1, 0, 0, 1);
    ...
    set_vector(p1.tex_coords, 1, 1, 0, 0);
    set_vector(p1.color, 0, 1, 0, 1);
    set_vector(p2.pos, 200, 200, 0, 1);
    set_vector(p2.tex_coords, 1, 0, 0, 0);
    set_vector(p2.color, 0, 0, 1, 1);
    set_vector(p3.pos, -200, 200, 0, 1);
    set_vector(p3.tex_coords, 0, 0, 0, 0);
    set_vector(p3.color, 1, 1, 0, 1);
    draw_barycentrictriangle(p0, p1, p2);
    draw_barycentrictriangle(p0, p2, p3);
}
...
void Tri(void) {
    ...
    vertex_list[2] = p2;
    num_verts = 3;
    triangle_list[0].index_list[0] = 2;
    triangle_list[0].index_list[1] = 1;
    triangle_list[0].index_list[2] = 0;
    num_triangles = 1;
    rotate_model(x_angle, y_angle, z_angle);
    project_model();
    scale_viewport(200, 200);
    draw_model();
}
...
void oneSphere(void) {
    init_sphere(0, 0, 0);
    rotate_model(x_angle, y_angle, z_angle);
    translate_model(0, 0, 3);
    perspectiveproject_model(2, 40);
    scale_viewport(300, 300);
    draw_model();
}

void oneRectangle(void) {
    init_quad();
    rotate_model(x_angle, y_angle, z_angle);
    translate_model(0, 0, 3);
    perspectiveproject_model(2, 40);
...
static void Key(unsigned char key, int x, int y)
{
    ...
    case 'm':
        texOrRGBA += 1;
        texOrRGBA = texOrRGBA % 2;
        draw_one_frame = 1;
        glutPostRedisplay();
        break;
    case 'p':
        perspective_correction += 1;
        perspective_correction = perspective_correction % 2;
        draw_one_frame = 1;
        glutPostRedisplay();
        break;
    case 'q':
