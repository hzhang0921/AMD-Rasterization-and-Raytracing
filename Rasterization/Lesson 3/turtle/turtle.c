...
/* 
 * turtle_draw_square_recursive() 
 */
void turtle_draw_square_recursive(int length) {
    if (length < 2) {
        return;
    }
    for (int i = 0; i < 4; i++) {
        ...
        turtle_right(90); 
        turtle_draw_square_recursive(length / 2); // draw another square at the start of each corner in the square.
    }
}
...
/*
 * turtle_draw_v_recursive()
 */
void turtle_draw_v_recursive(int length) {
    if (length < 1) {
        return;
    }
    turtle_left(30); 
    turtle_forward(length);
    turtle_draw_v_recursive(length / 2);
...
/*
 * draw_mountain_recursive()
 */
void turtle_draw_mountain_recursive(int length) {
    if (length < 1) {
        return;
    }
    turtle_right(90);
    ...
    turtle_left(45);
    turtle_forward(length);
    turtle_draw_mountain_recursive(length / 2);
    turtle_right(90);
    turtle_forward(length);
    turtle_left(45);
    turtle_forward(length);
...
void random_walk() {
    for (int i = 0; i < 50000; i++) {
        int angle = (int)(random_float(-30, 30));
        int movement = (int)(random_float(0, 5));
        turtle_right(angle);
        turtle_forward(movement);
        if (tcur.pos[X] > 400) {
            turtle_backward(movement);
            cur_heading = 180.0 - cur_heading;
            turtle_forward(movement);
        } else if (tcur.pos[X] < -400) {
            turtle_backward(movement);
            cur_heading = 180.0 - cur_heading;
            turtle_forward(movement);
        }
        if (tcur.pos[Y] > 400) {
            turtle_backward(movement);
            cur_heading = 360.0 - cur_heading;
            turtle_forward(movement);
        } else if (tcur.pos[Y] < -400) {
            turtle_backward(movement);
            cur_heading = 360.0 - cur_heading;
            turtle_forward(movement);
        }
...
void display( void )
{
    ...
    if (test == 0) {
        ...
        turtle_draw_square_recursive(200);
    }
    else if (test == 13) {
        turtle_draw_v_recursive(100);
    }
    else if (test == 14) {
        turtle_draw_mountain_recursive(100);
    }
...
static void Key( unsigned char key, int x, int y )
{
    ...
    {     
		...
		case 't':       test++; if( test > 14 ) test = 0; draw_one_frame = 1;     glutPostRedisplay();    break;
        case 'y':       test--; if (test < 0 ) test = 14; draw_one_frame = 1;     glutPostRedisplay();    break;
		case 'q':       exit( 0 );                             											 break;
